...
interface Props {
  ...
  onSwap: (taskAId: string, taskBId: string) => void
  onDelete: (id: string) => void
}

export function Manage({ tasks, onAdd, onUpdate, onSwap, onDelete }: Props) {
  ...
  function moveTask(index: number, direction: 'up' | 'down') {
    ...
    onSwap(activeTasks[index].id, activeTasks[swapIndex].id)
  }
              ...
              {reordering && (
                <div className="flex flex-col gap-0.5">
                  <button
                    ...
                    disabled={index === 0}
                    className="rounded p-1 text-slate-400 hover:bg-slate-700 hover:text-slate-200 disabled:opacity-20 disabled:hover:bg-transparent disabled:hover:text-slate-400"
                    title="Move up"
                  >
